...
describe("crypto capital deployed", () => {
  it("sums buy amounts across holdings (fees excluded)", () => {
    ...
    expect(calculateHoldingContribution(holdings[0])).toBe(500);
    expect(calculateCryptoCapitalDeployed(holdings)).toBe(900);
  });
  ...
  it("treats missing fees as zero", () => {
    ...
    expect(calculateCryptoCapitalDeployed(holdings)).toBe(500);
  });
});
